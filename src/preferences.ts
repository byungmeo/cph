...
export const getCsArgsPref = (): string[] =>
    getPreference('language.cs.Args').split(' ') || [];
...
export const getCsCommand = (): string =>
    getPreference('language.cs.Command') || 'dotnet';
...
export const getLanguageId = (srcPath: string): number => {
    ...
    switch (extension) {
        ...
        case '.cs': {
            compiler = getPreference('language.cs.SubmissionCompiler');
            break;
        }
    }
